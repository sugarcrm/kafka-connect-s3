...
@Ignore(
    "Doesn't seem to work on CircleCI 2.0 - I believe it has to do with the Docker Client not being"
        + " compatible")
public class S3ConnectorIntegrationTest {
  ...
  public static void startKafka() throws Exception {
    ...
    stringConnect =
        givenKafkaConnect(
            kafka.localPort(), ImmutableMap.of("value.converter", StringConverter.class.getName()));
  ...
  public void cleanup() {
    ...
    connect.herder().connectors((e, connectors) -> connectors.forEach(this::whenTheSinkIsStopped));
  }
  ...
  private void whenTheSinkIsStopped(String name) {
    connect
        ...
        .putConnectorConfig(
            ...
            true,
            (e, c) -> {
              if (e != null) Throwables.throwIfUnchecked(e);
            });
  }

  // this seems to take at least 20 seconds to get a segment deleted :(
  private Map.Entry<String, List<Long>> slow_givenATopicWithNonZeroStartOffsets()
    ...
    ImmutableList<Long> offsets =
        waitForPassing(
            Duration.ofMinutes(10),
            () -> {
              consumer.seekToBeginning(
                  ImmutableList.of(new TopicPartition(topic, 0), new TopicPartition(topic, 1)));
  ...
  public void binaryWithKeys() throws Exception {
    ...
    // This will fail if we get duplicates into the sink topic, which can happen because we have
    // duplicates
  ...
  private void thenTempFilesAreCleanedUp(Map<String, String> sinkConfig) {
    ...
    waitForPassing(
        ...
        () ->
            assertEquals(
                ImmutableList.of(),
                ImmutableList.copyOf(new File(sinkConfig.get("local.buffer.dir")).list())));
  }
  ...
  private Map<String, String> givenStringValues(Map<String, String> config) {
    ...
    copy.remove("format");
    copy.putAll(ImmutableMap.of("value.converter", StringConverter.class.getName()));
    return copy;
  }
  ...
  public void givenMoreData(String sinkTopic, Producer<String, String> producer)
      throws InterruptedException, ExecutionException, TimeoutException {
  ...
  private Consumer<String, String> givenAConsumer() {
    return new KafkaConsumer<>(
        ImmutableMap.of("bootstrap.servers", "localhost:" + kafka.localPort()),
  ...
  private void whenTheConnectorIsStarted(
      Map<String, String> config, KafkaIntegrationTests.KafkaConnect connect) {
    connect.herder().putConnectorConfig(config.get("name"), config, false, (e, s) -> {});
  }
  ...
  private void thenFilesAreWrittenToS3(AmazonS3 s3, String sinkTopic, List<Long> offsets) {
    waitForPassing(
        ...
        () -> {
          List<String> keys =
              s3.listObjects(S3_BUCKET, S3_PREFIX).getObjectSummaries().stream()
                  .map(S3ObjectSummary::getKey)
                  .collect(toList());
          Set<Map.Entry<Integer, Long>> partAndOffset =
              keys.stream()
                  .filter(key -> key.endsWith(".gz") && key.contains(sinkTopic))
                  .map(
                      key ->
                          immutableEntry(
                              Integer.parseInt(key.replaceAll(".*?-(\\d{5})-\\d{12}\\.gz", "$1")),
                              Long.parseLong(key.replaceAll(".*?-\\d{5}-(\\d{12})\\.gz", "$1"))))
                  .collect(toSet());

          Map<Integer, Long> startOffsets =
              transformValues(
                  partAndOffset.stream()
                      .collect(groupingBy(Map.Entry::getKey, minBy(Map.Entry.comparingByValue()))),
                  (optEntry) -> optEntry.map(Map.Entry::getValue).orElse(0L));

          assertTrue(
              startOffsets + "[0] !~ " + offsets,
              ofNullable(startOffsets.get(0)).orElse(-1L) >= offsets.get(0));
          assertTrue(
              startOffsets + "[1] !~ " + offsets,
              ofNullable(startOffsets.get(1)).orElse(-1L) >= offsets.get(1));
        });
  }
  ...
  private Map<String, String> givenSourceConfig(String sourceTopic, String sinkTopic)
      throws IOException {
    return s3Config(
  ...
  private ImmutableMap.Builder<String, String> s3Config(
  ...
  private void givenRecords(String originalTopic, Producer<String, String> producer)
  ...
  private void givenRecords(String originalTopic, Producer<String, String> producer, int start)
    ...
    IntStream.range(start, start + 4)
        .forEach(
            i -> {
              try {
                producer
                    .send(new ProducerRecord<>(originalTopic, i % 2, "key:" + i, "value:" + i))
  ...
  private void thenMessagesAreRestored(String sourceTopic, int start, AmazonS3 s3) {
    ...
    ImmutableList<TopicPartition> partitions =
        ImmutableList.of(new TopicPartition(sourceTopic, 0), new TopicPartition(sourceTopic, 1));
    consumer.assign(partitions);
    consumer.seekToBeginning(partitions);

    // HACK - add to this list and then assert we are done. will retry until done.
    List<List<String>> results = ImmutableList.of(new ArrayList<>(), new ArrayList<>());
    waitForPassing(
        ...
        () -> {
          ...
          assertEquals(
              "got all the records for 0 " + results,
              2,
              distinctAfter(start, results.get(0)).count());
          assertEquals(
              "got all the records for 1 " + results,
              2,
              distinctAfter(start, results.get(1)).count());
        });
    ...
    ImmutableList<List<String>> actual =
        ImmutableList.of(startOdd ? odds : evens, startOdd ? evens : odds);
    if (!expected.equals(actual)) {
                  ...
                  idx -> {
                    try {
                      return Stream.concat(
                          ...
                          ((ChunksIndex)
                                  new ObjectMapper()
                                      .readerFor(ChunksIndex.class)
                                      .readValue(
                                          s3.getObject(S3_BUCKET, idx.getKey()).getObjectContent()))
                              .chunks.stream()
                                  .map(
                                      c ->
                                          String.format(
                                              "[%d..%d]",
                                              c.first_record_offset,
                                              c.first_record_offset + c.num_records)));
                    } catch (IOException e) {
                      return null;
                    }
                  })
      ...
      // NOTE: results is asserted to assist in debugging. it may contain values that should be
      // skipped
      assertEquals(
          "records restored to same partitions, in-order. Chunks = " + chunks, expected, results);
    }
