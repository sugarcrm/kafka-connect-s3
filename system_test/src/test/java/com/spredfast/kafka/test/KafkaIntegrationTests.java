...
public class KafkaIntegrationTests {
  ...
  public static KafkaConnect givenKafkaConnect(
      int kafkaPort, Map<? extends String, ? extends String> overrides) throws IOException {
    File tempFile = java.nio.file.Files.createTempFile("connect", "offsets").toFile();
  ...
  private static KafkaConnect givenKafkaConnect(Map<String, String> props) {
    ...
    Worker worker =
        new Worker(
            "1", new SystemTime(), new ConnectorFactory(), config, new FileOffsetBackingStore());
  ...
  public static class Kafka implements AutoCloseable {
    ...
    public Kafka() throws Exception {
      ...
      kafkaServer =
          new KafkaServer(
              ...
              SystemTime$.MODULE$,
              Option.empty(),
              JavaConversions.asScalaBuffer(ImmutableList.of()));
      kafkaServer.startup();
    }
    ...
    public String createUniqueTopic(String prefix, int partitions, Properties topicConfig)
      ...
      checkReady();
      String topic =
          (prefix + UUID.randomUUID().toString().substring(0, 5))
      ...
      waitForPassing(
          ...
          () -> {
            assertTrue(
                AdminUtils.fetchTopicMetadataFromZk(topic, kafkaServer.zkUtils())
                    .partitionMetadata()
                    .stream()
