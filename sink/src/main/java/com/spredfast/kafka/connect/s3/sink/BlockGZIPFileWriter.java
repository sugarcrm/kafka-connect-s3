...
/**
 * BlockGZIPFileWriter accumulates newline delimited UTF-8 records and writes them to an output file
 * that is readable by GZIP.
 *
 * <p>In fact this file is the concatenation of possibly many separate GZIP files corresponding to
 * smaller chunks of the input. Alongside the output filename.gz file, a file filename-index.json is
 * written containing JSON metadata about the size and location of each block.
 *
 * <p>This allows a reading class to skip to particular line/record without decompressing whole file
 * by looking up the offset of the containing block, seeking to it and beginning GZIp read from
 * there.
 *
 * <p>This is especially useful when the file is an archive in HTTP storage like Amazon S3 where GET
 * request with range headers can allow pulling a small segment from overall compressed file.
 *
 * <p>Note that thanks to GZIP spec, the overall file is perfectly valid and will decompress as if
 * it was a single stream with any regular GZIP decoding library or program.
 */
public class BlockGZIPFileWriter implements Closeable {
  ...
  public BlockGZIPFileWriter(File directory, long startOffset, long chunkThreshold)
      throws IOException {
  ...
  public BlockGZIPFileWriter(File directory, long startOffset, long chunkThreshold, byte[] header)
    ...
    dataFile = Files.createTempFile(directory.toPath(), "data", null).toFile();
    indexFile = Files.createTempFile(directory.toPath(), "index", null).toFile();
  ...
  private void writeIndex() throws IOException {
    ...
    objectMapper
        .writer()
        .writeValue(
            indexFile, ChunksIndex.of(chunks.stream().map(Chunk::toJson).collect(toList())));
  }
