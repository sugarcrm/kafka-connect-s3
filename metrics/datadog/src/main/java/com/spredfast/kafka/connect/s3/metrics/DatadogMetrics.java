 ...
 *
 * <p>Config: https://github.com/spredfast/kafka-connect-s3/wiki/Metrics
 */
public class DatadogMetrics implements Metrics {
  ...
  public DatadogMetrics(Map<String, String> config) {
    ...
    ofNullable(config.get("prefix")).ifPresent(builder::withPrefix);
    ...
    builder
        .build()
        .start(Integer.parseInt(config.getOrDefault("frequency", "10")), TimeUnit.SECONDS);
  }
  ...
  private Transport buildUdp(Map<String, Object> transport) {
    ...
    ofNullable(transport.get("host")).map(Object::toString).ifPresent(builder::withStatsdHost);
    ofNullable(transport.get("prefix")).map(Object::toString).ifPresent(builder::withPrefix);
  ...
  private Transport buildHttp(Map<String, Object> transport) {
    ...
    ofNullable(transport.get("proxy.port"))
        ...
        .ifPresent(
            port ->
                builder.withProxy(
                    transport.getOrDefault("proxy.host", "localhost").toString(), port));
