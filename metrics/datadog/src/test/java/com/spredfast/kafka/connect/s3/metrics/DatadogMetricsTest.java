...
public class DatadogMetricsTest {
  ...
  public void metricsAreTagged() {
    ...
    assertEquals(
        ImmutableSet.of("foo[foo:bar]", "foo[foo:baz]"),
        metrics.getRegistry().getHistograms().keySet());

    // and order our tags correctly
    assertEquals(
        ImmutableSet.of("fooMeter[bar:qux,foo:baz]"), metrics.getRegistry().getMeters().keySet());
  }

  @Test
  public void parseTags() {
    assertEquals(
        Optional.of(ImmutableList.of("a:b", "c:d", "longer_name:has-a-value")),
        DatadogMetrics.parseTags(ImmutableMap.of("tags", "a:b,c:d,longer_name:has-a-value")));

    assertEquals(Optional.empty(), DatadogMetrics.parseTags(ImmutableMap.of()));

    assertEquals(Optional.empty(), DatadogMetrics.parseTags(ImmutableMap.of("tags", "")));
  }
}
