...
public class TrailingDelimiterFormatTest {
  ...
  public void defaults() throws IOException {
    FormatTests.roundTrip_singlePartition_fromZero_withNullKeys(
        givenFormatWithConfig(ImmutableMap.of()), givenValues());
  }

  @Test
  public void withKeys() throws IOException {
    FormatTests.roundTrip_singlePartition_fromZero_withKeys(
        givenFormatWithConfig(ImmutableMap.of("key.delimiter", "\t")),
        ImmutableMap.of(
            "123", "456",
            "abc\ndef", "ghi\tjkl"),
        0);
  ...
  public void output() throws IOException {
    // using UTF_16BE because UTF_16 adds a byte order mark to every invocation of getBytes, which
    // is annoying
    TrailingDelimiterFormat format =
        givenFormatWithConfig(
            ImmutableMap.of(
                "key.delimiter", "\t",
  ...
  private ImmutableList<String> givenValues() {
    return ImmutableList.of(
        "abcd",
        "567\tav",
        "2384732109847123098471092384723109847239084732409854329865293847549837");
  }
