...
/**
 * Encodes raw bytes, prefixed by a 4 byte, big-endian integer indicating the length of the byte
 * sequence.
 */
public class ByteLengthFormat implements S3RecordFormat, Configurable {
  ...
  public void configure(Map<String, ?> configs) {
    includesKeys =
        Optional.ofNullable(configs.get("include.keys"))
            .map(Object::toString)
            .map(Boolean::valueOf)
            .filter(f -> f);
  }
  ...
  private byte[] encode(ProducerRecord<byte[], byte[]> r) {
    ...
    byte[] result =
        new byte
            [LEN_SIZE + value.length + (includesKeys.map(t -> key.length + LEN_SIZE).orElse(0))];
    ByteBuffer wrapped = ByteBuffer.wrap(result);
    includesKeys.ifPresent(
        t -> {
