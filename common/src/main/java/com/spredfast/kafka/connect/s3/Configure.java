...
public abstract class Configure {
  ...
  private static final Class<? extends S3RecordFormat> DEFAULT_FORMAT =
      TrailingDelimiterFormat.class;

  private static final Map<?, String> FORMAT_ALIAS =
   ...
   * @param classNameProp the name of the property that specifies the converter class. Any sub
   *     properties will be passed to the converter as config.
   * @param isKey if this converter is for a key or not.
   * @param defaultConverterClass the default converter class to create and configure if the prop is
   *     missing. If null, the result may be null.
   * @return a new converter, already configured.
   */
  public static Converter buildConverter(
      Map<String, ?> props,
      String classNameProp,
      boolean isKey,
      Class<? extends Converter> defaultConverterClass) {
    ...
    try {
      ...
      throw new IllegalArgumentException(
          "Could not create S3 converter for props " + classNameProp + " isKey=" + isKey, e);
    }
  }
  ...
  public static Metrics metrics(Map<String, String> props) {
    return ofNullable(props.get("metrics.reporter"))
        .map(
            className ->
                "datadog".equals(className)
                    ? "com.spredfast.kafka.connect.s3.metrics.DatadogMetrics"
                    : className)
        .map(
            className ->
                Metrics.getByName(
                    props.getOrDefault("metrics.reporter.name", ""),
                    clazz(className),
                    subStringKeys("metrics.reporter", props)))
        .orElse(Metrics.NOOP);
  }
  ...
  public static S3RecordFormat createFormat(Map<String, String> props) {
    try {
      S3RecordFormat recordFormat =
          (S3RecordFormat)
              ofNullable(props.get("format"))
                  .map(Object::toString)
